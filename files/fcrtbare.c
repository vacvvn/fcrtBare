...
MODULE_DESCRIPTION("fcrtBare - loadable module template generated by petalinux-create -t "
                   "modules");
...
///дескрипторы разбиваются на пары(0-1); (2-3); .... (20-21)
// #define VCD_IND_CROSSING
...
static void show_rxq_elem(struct fcrtRxQueue *ptr)
{
    ...
    printk(KERN_ALERT "asm_id: 0x%x; q_depth: %d; msg_max_len: %d", ptr->asm_id,
           ptr->q_len, ptr->max_m_len);
    printk(KERN_ALERT "queue addr");
    for (j = 0; j < ptr->q_len; j++)
    {
        printk(KERN_ALERT "[%d]:v.addr: %p; m_len: %d", j, ptr->rx_q[j],
               ptr->rx_m_len[j]);
    }
}
...
#endif
{
    ...
    rx_q = fcrt_alloc(vc_cnt * (sizeof(struct fcrtRxQueue)), sizeof(struct fcrtRxQueue),
                      &no_use);
    if (rx_q == NULL)
    {
...
#ifdef VCD_IND_CROSSING
    vcd_ind ^= 0x01u;
    if(vcd_ind >= vc_cnt)
    {
        printk(KERN_ALERT"[%s]crossed vcd_ind is invalid: %d", __func__, vcd_ind);
        return -EINVAL;
    }
#endif
    ptr = &rx_q[vcd_ind];
